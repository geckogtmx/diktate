...
class SystemMonitor:
    ...
    def _initialize_gpu(self) -> None:
        ...
        if not TORCH_AVAILABLE:
            logger.debug(
                "[SystemMonitor] Torch not found - relying on nvidia-smi for GPU detection"
            )
        ...
        if self.has_nvidia_gpu:
            try:
                ...
                self.gpu_device_name = (
                    torch.cuda.get_device_name(0) if self.gpu_device_count > 0 else "Unknown"
                )
                ...
                logger.info(
                    f"[SystemMonitor] Detected GPU: {self.gpu_device_name} ({self.gpu_total_memory:.2f} GB)"
                )
    ...
    def _get_gpu_metrics_smi(self) -> dict[str, float | None]:
    ...
    def get_snapshot(self) -> dict:
        ...
        return {
            ...
            "gpu_memory_percent": round(gpu_memory_percent, 1)
