...
def play_audio_with_timeout(segment, timeout_multiplier=1.5):
    ...
    if thread.is_alive():
        print(f"  [ERROR] Playback timeout after {timeout:.1f}s - audio may still be playing in background")
        print(f"  [INFO] Simulating playback completion...")
        return False
...
def play_audio_segment(segment):
    ...
    if USE_SIMPLEAUDIO:
        try:
            ...
            print(f"  [INFO] Disabling simpleaudio for remaining phrases...")
    ...
    try:
        ...
        print(f"  [DEBUG] Starting PyAudio playback...")
        ...
        print(f"  [DEBUG] PyAudio playback completed")
        return True

    except ImportError:
        print(f"  [ERROR] PyAudio not installed, falling back to pydub.playback...")
        # Last resort: pydub.playback (can hang on Windows)
        try:
            from pydub.playback import play
            print(f"  [DEBUG] Starting pydub.playback.play()...")
            play(segment)
            print(f"  [DEBUG] pydub.playback.play() completed")
            return True
        except Exception as e3:
            print(f"  [ERROR] pydub.playback.play failed: {e3}")
            print(f"  [ERROR] Skipping audio playback for this phrase")
...
def smart_feed(audio_path: Path, srt_path: Path, loop: bool = False, count: int = 0, start_at: int = 0):
    ...
    try:
        ...
        print(f"[OK] Audio validated: {duration_ms/1000:.1f}s, max amplitude: {max_amplitude}")

        if max_amplitude < 100:
            print(f"[WARNING] Audio appears very quiet (max amplitude: {max_amplitude})")
            print("This may affect transcription quality. Press Enter to continue or Ctrl+C to abort...")
            input()
    except Exception as e:
    ...
    print(f"Merging short subtitles into longer phrases (min 8s)...")
    merged_subs = []
    current_group = None
    MIN_PHRASE_DURATION_MS = 8000  # 8 seconds minimum

    for sub in subs:
        duration_ms = (sub.end.hours * 3600 + sub.end.minutes * 60 + sub.end.seconds) * 1000 + sub.end.milliseconds
        duration_ms -= (sub.start.hours * 3600 + sub.start.minutes * 60 + sub.start.seconds) * 1000 + sub.start.milliseconds
        ...
        # If current group is too short (< 8 seconds), merge this subtitle into it
            ...
            current_group['duration_ms'] = (current_group['end'].hours * 3600 + current_group['end'].minutes * 60 + current_group['end'].seconds) * 1000 + current_group['end'].milliseconds
            current_group['duration_ms'] -= (current_group['start'].hours * 3600 + current_group['start'].minutes * 60 + current_group['start'].seconds) * 1000 + current_group['start'].milliseconds
    ...
    while True:
        ...
        for i in range(start_at, total_subs):
            ...
            if start_ms < last_end_ms:
                ...
                print(f"  [SKIP] Overlaps with previous segment by {overlap_ms}ms - adjusting start time")
            ...
            print(f"\n{'='*60}")
            print(f"[{i+1}/{total_subs}] Phrase {completed+1}/{total_to_process} ({percent:.1f}%)")
            if completed > 0:
                print(f"Elapsed: {elapsed/60:.1f}m | ETA: {remaining/60:.1f}m | Avg: {avg_per_phrase:.1f}s/phrase")
            print(f"{'='*60}")
            print(f"  [EXPECTED] \"{expected_text[:80]}{'...' if len(expected_text) > 80 else ''}\"")
            print(f"  [TIMING] {start_ms/1000:.2f}s -> {end_ms/1000:.2f}s (Duration: {duration_ms/1000:.2f}s)")
            ...
            try:
                ...
                print(f"  [INFO] Will still send STOP command...")
            finally:
            ...
            if not wait_for_idle(timeout=30.0):
                ...
                print(f"  [PAUSE] Waiting 1.0s before next phrase...")
                time.sleep(1.0)
...
def dumb_feed(audio_path: Path, chunk_len_sec: int = 10, loop: bool = False):
    ...
    while True:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if i > 0:
                print(f"Elapsed: {elapsed/60:.1f}m | ETA: {remaining/60:.1f}m | Avg: {avg_per_chunk:.1f}s/chunk")
            print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-simpleaudio", action="store_true", help="Force use of pydub.playback.play (slower but more stable)")
