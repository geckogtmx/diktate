...
\- \*\*Snippets\*\* - Voice macros that expand short phrases into predefined text blocks (v1.1).

\- \*\*Voice Commands\*\* - Control application functions and modes entirely via voice (v1.2).

\- \*\*Streamer Satellite\*\* - AI co-host tools for broadcasters including chat interaction (Planned).
...
\- \*\*Performance Metrics\*\* - Real-time dashboard usage tracking, latency breakdown, and speed stats.
...
\- \*\*One-Time Purchase\*\* - Lifetime access for a single flat fee ($10 Libre / $25 Pro).
...
\- \*\*Google Login Integration\*\* - OAuth support to utilize existing Google AI Pro/Ultra subscription quotas.

\- \*\*BYO Keys\*\* - Plug in personal OpenAI, Anthropic, or DeepSeek API keys for cloud processing.

\- \*\*Wallet Credits\*\* - Optional prepaid system for cloud-based transcription backup (v1.1).
