...
export async function initializePrivacySettings() {
    ...
    wipeBtn?.addEventListener('click', async () => {
        const confirmed = confirm("Are you sure you want to permanently delete all local history and metrics? This cannot be undone.");
        if (confirmed) {
            try {
...
export function loadPrivacySettings(settings: any) {
    ...
    const intensity = settings.privacyLoggingIntensity !== undefined ? settings.privacyLoggingIntensity : 2;
